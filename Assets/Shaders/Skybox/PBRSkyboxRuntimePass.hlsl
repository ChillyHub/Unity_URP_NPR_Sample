...
Varyings PBRSkyboxPassVertex(Attributes input)
{
    ...
    ScatteringData data = GetScatteringData(_LightColor, RADIUS, THICKNESS, _RScale, _Exposure, _gMie,
...
float4 PBRSkyboxPassFragment(Varyings input) : SV_Target
{
    ...
    float3 sun = min(GetSunRenderColor(_SunColor, light.direction, viewDir, _gSun), FLT_MAX);
